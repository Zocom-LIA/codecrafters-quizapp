...
// --- QUIZZES PAGE LOGIC ---
//
async function handquizzeslistsPage() {
    const userId = sessionStorage.getItem('userId');
    const role = sessionStorage.getItem('userRole'); // Ensure the role is fetched correctly
    const errorMessage = document.createElement('p'); // Define errorMessage dynamically
...
function displayQuizzes(quizzes) {
    ...
    quizzes.forEach((quiz) => {
        ...
        editButton.addEventListener('click', () => {
            ...
            window.location.href = ''; // Redirect to farah page
        });
